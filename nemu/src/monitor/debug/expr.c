...
enum {
    NOTYPE = 256, EQ, NUM, HEX, REG, NEQ, AND, OR, NOT, MINUS, POINTER, SYMBOL
...
static struct rule {
    ...
    int token_type;
        ...
        {"\\|\\|",                  OR},
        {"\\$[a-zA-z]+",            REG},        // register
        {"\\b0[xX][0-9a-fA-F]+\\b", HEX}, // hexnumber
        {"\\b[0-9]+\\b",            NUM},                // number
        {"-",                       '-'},                        // minus
        {"\\*",                     '*'},                    // multiply
        {"/",                       '/'},                        // divide
...
//				Log("match rules[%d] = \"%s\" at position %d with len %d: %.*s", i, rules[i].regex, position, substr_len, substr_len, substr_start);
                ...
                switch (rules[i].token_type) {
                    ...
                    default: {
                        ...
                        if (rules[i].token_type == REG) { //Register
...
uint32_t eval(int l, int r) {
    ...
    if (l == r) {
        uint32_t num = 0;
        if (tokens[l].type == SYMBOL) {
            num = get_symbol_addr(tokens[l].str, cando);
            if (*cando == false) return 0;
    ...
    if (check_bracket(l, r)) return eval(l + 1, r - 1);
        ...
        if (l == pos || tokens[pos].type == NOT || tokens[pos].type == MINUS || tokens[pos].type == POINTER) {
            ...
            switch (tokens[pos].type) {
                case POINTER:
                    return swaddr_read(r_ans, 4, R_DS);
                case NOT:
                    return !r_ans;
                case MINUS:
                    return -r_ans;
...
uint32_t expr(char *e, bool *success) {
    ...
    for (i = 0; i < nr_token; i++) {
        ...
        if (tokens[i].type == '-' && (i == 0 || (tokens[i - 1].type != ')' && tokens[i - 1].type != NUM &&
                                                 tokens[i - 1].type != HEX && tokens[i - 1].type != REG))) {
            tokens[i].type = MINUS;
        }
        if (tokens[i].type == '*' && (i == 0 || (tokens[i - 1].type != ')' && tokens[i - 1].type != NUM &&
                                                 tokens[i - 1].type != HEX && tokens[i - 1].type != REG))) {
